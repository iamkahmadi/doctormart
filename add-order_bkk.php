            ...
            <div class="col-lg-11" style="    margin-left: 5%;">
                            ...
                            <form class="form-horizontal" method="POST" id="createOrderForm" action="php_action/order.php" action="php_action/order.php">
                                ...
                                <div class="form-group">
                                    <div class="row">
                                        ...
                                        <div class="col-sm-4">
                                            <input type="text" class="form-control" id="clientContact" name="clientContact" placeholder="Contact Number No." autocomplete="off" required />
                                        </div>
                ...
                <div class="form-group">
                    <div class="row">
                        ...
                        <div class="col-sm-4">
                            <input type="number" min="0" class="form-control" id="discount" name="discount" onkeyup="discountFunc()" autocomplete="off" / pattern="^[0-9]+$" />
                        </div>

                        <input type="hidden" class="form-control" id="grandTotal" name="grandTotal" disabled="true" />
...
<script>
    ...
    function discountFunc() {
        var discount = $("#discount").val(); // Discount entered by user
        var totalAmount = Number($("#totalAmount").val()); // Total amount

        totalAmount = totalAmount.toFixed(2); // Make sure it's a fixed value for consistency
        ...
        if (discount && totalAmount) {
            // Calculate discount as a percentage of totalAmount
            var discountValue = (totalAmount * (Number(discount) / 100)).toFixed(2);

            // Subtract the discount from the totalAmount
            grandTotal = (totalAmount - discountValue).toFixed(2);

            // Set the grandTotal values
            $("#grandTotal").val(grandTotal);
            $("#grandTotalValue").val(grandTotal);
        } else {
            // If no discount or total amount, set grandTotal equal to totalAmount
            grandTotal = totalAmount;
            $("#grandTotal").val(grandTotal);
            $("#grandTotalValue").val(grandTotal);
        }

        var paid = $("#paid").val(); // Amount paid by the user
        var dueAmount;

        if (paid) {
            // Calculate the remaining due amount
            dueAmount = (Number(grandTotal) - Number(paid)).toFixed(2);
            $("#due").val(dueAmount);
            $("#dueValue").val(dueAmount);
        } else {
            // If no amount paid, due is equal to grandTotal
            $("#due").val(grandTotal);
